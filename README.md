...
- 访客填自己的 API key 时，翻译请求由浏览器**直接**发给模型厂商，不经过本站服务器。
  key 不在服务端落地，也不进日志 —— 这一点在网络面板里可以自己验证。
- 服务端只做三件事：取字幕、管试用额度、限流。它拿不到访客的 key。
- 没有自带 key 的访客走站长配置的试用额度，由三重上限封死支出（单 IP 每日次数、
  全站每日总次数、单次字幕条数）。

**字幕来源只有 Supadata 一条。** 早期还有一条 yt-dlp 的非官方抓取路径，从机房 IP 会被
YouTube 的机器人检测持续拦截 —— 实测每个视频都要先失败一轮、白等十几秒，而且失败时
的报错还会盖住真正的原因，已整条移除。

请求固定带 `mode=native`，只取视频**已有**的字幕。默认的 `auto` 会在没字幕时转去调用
AI 转录，那是按分钟计费的（每分钟 2 credits）—— 一部 90 分钟的访谈要 180 credits，
一次就超过整月免费额度。

**成本参考**（实测 token 量，按 `gpt-4o-mini` 计价）：一部 1.5 小时的访谈约 $0.03。
翻译量最大且是机械转换，所以它和判断类任务（出题、点评）用不同档位的模型，
共用会贵二十倍左右。
...
- 模拟面试走本地题库和规则评分
- 正文抽取走规则解析
- YouTube 字幕**读取**照常（字幕来源是 Supadata，与 OpenAI 无关）
- 但字幕**翻译**需要模型：访客可以在页面上填自己的 key 自助使用；
  服务端不配 key 就没有免费试用额度可发
...
本项目以 [MIT 协议](./LICENSE) 开源，可自由使用、修改和分发，保留版权声明即可。

YouTube 工具里的**章节速览、金句提取、划词解释**几个功能，思路参考了
